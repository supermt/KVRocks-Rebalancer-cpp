...
if [ "$1" == "start" ]; then
  ...
  while [ $((PORT < TOTALPORT)) != "0" ]; do
    PORT=$((PORT + 1))
    echo "Starting $PORT"
    mkdir ${DB_PATH_PREFIX}/node_${PORT}
    ...
    sed -i.bak "s|dir.*|dir "${DB_PATH_PREFIX}node_${PORT}"|g" ${conf_file} && rm ${conf_file}.bak
    sed -i.bak "s|migrate-method.*|migrate-method "$2"|g" ${conf_file} && rm ${conf_file}.bak
...
if [ "$1" == "create" ]; then
  ...
  PORT=$START_PORT
  cluster_nodes=""
  mkdir -p -m 777 /tmp/migration_sync/
