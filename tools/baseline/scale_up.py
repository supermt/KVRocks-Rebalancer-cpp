...
for migration_method in [0,2,3]:
    for workload in ['i']:  # ,'b','c','d','e','f']:
        ...
        if len(RSLIST_int) == 0:
            ...
            print("loading")
            os.system("./loading.sh "+workload+" > " + DIR_int + "load_result")

            print("migrating and running")

            os.system("redis-cli -h 127.0.0.1 -p 40001 stats >" + DIR_int + "/init_run.node1.stat")
            os.system("redis-cli -h 127.0.0.1 -p 40002 stats >" + DIR_int + "/init_run.node2.stat")

            os.system("./running_while_migrating.sh " + workload + " " + str(migration_method) + " > " + DIR_int + "run_result")
            print(">>>>>>>>>>>>>>>>>>>Cleaning<<<<<<<<<<<<<<<<<<<\n\n")
            os.system("redis-cli -h 127.0.0.1 -p 40001 stats >" + DIR_int + "/run_finished.node1.stat")
            os.system("redis-cli -h 127.0.0.1 -p 40002 stats >" + DIR_int + "/run_finished.node2.stat")
